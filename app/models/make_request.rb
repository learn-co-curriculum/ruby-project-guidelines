...
def self.populate_match
    ...
    match_filtered.each do |m|
        Match.create(date: m[0], home_team_id: Team.find_by(name:"#{m[1]}").id, away_team_id: Team.find_by(name:"#{m[2]}").id, home_team_goals: m[3], away_team_goals: m[4], arena_id: Arena.find_by(name: "#{m[5]}").id)
    end
end
